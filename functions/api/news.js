const FEEDS = [
  ...
  { source: 'MSN Money', category: 'markets', url: 'https://news.google.com/rss/search?q=MSN%20Money%20markets%20economy&hl=en-US&gl=US&ceid=US:en' },
  { source: 'Truth Social', category: 'geopolitics', url: 'https://news.google.com/rss/search?q=Truth%20Social%20Trump%20tariffs%20Fed%20oil%20markets&hl=en-US&gl=US&ceid=US:en' }
];
...
const assets = [
  ['USD',['fed','dollar','rate','fomc','treasury','cpi','nfp']],
  ...
  ['Stocks',['stocks','nasdaq','s&p','dow','earnings']]
];
...
function analyze(item){
  ...
  return {
    ...
    sentiment: /(falls|drops|war|attack|weak|recession|sanction)/i.test(text) ? 'bearish' : /(rises|gains|strong|growth|beats|surges)/i.test(text) ? 'bullish' : 'neutral',
...
async function fetchFeed(feed){
  try{
    const res = await fetch(feed.url, { cf: { cacheTtl: 60, cacheEverything: false }, headers: { 'user-agent': 'HawaliAburiBot/1.0' }});
    if(!res.ok) throw new Error(String(res.status));
    const xml = await res.text();
    const entries = [...xml.matchAll(/<item[\s\S]*?<\/item>/gi)].slice(0,8).map(m=>m[0]);
    return entries.map((entry, idx)=>{
      ...
      const item = { id: `${feed.source}-${idx}-${title}`.slice(0,180), title, summary, source: feed.source, category: feed.category, link, publishedAt, image };
...
export async function onRequest() {
  ...
  let items = batches.flat().filter(i=>{
    const key = i.title.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim().slice(0,90);
