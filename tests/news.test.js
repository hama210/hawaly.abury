...
test('news sources are curated around the focused markets and wars', () => {
  ...
  assert.ok(names.includes('NPR World'))
  ...
  assert.equal(FEEDS.find(feed => feed.source === 'NPR World')?.url, 'https://feeds.npr.org/1004/rss.xml')
  assert.equal(FEEDS.find(feed => feed.source === 'NPR World')?.format, 'iran-us-direct')
