...
const nav = [
  ['dashboard', Home], ['latest', Newspaper], ['geopolitics', ShieldAlert], ['iraq', Landmark], ['forex', BarChart3], ['calendar', CalendarDays], ['crypto', Bitcoin], ['oil', Fuel], ['stocks', TrendingUp], ['banks', Building2], ['ai', Brain], ['settings', Settings]
]
...
export default function Sidebar({ t, open, setOpen, activeCategory, setActiveCategory }){
  return <aside className={`sidebar ${open?'open':''}`}>
    ...
    <nav>
      {nav.map(([key, Icon]) => {
        const categoryMap = { dashboard:'all', latest:'all', geopolitics:'geopolitics', iraq:'iraq', forex:'forex', calendar:'calendar', crypto:'crypto', oil:'oil', stocks:'stocks', banks:'banks', ai:'all' }
        const cat = categoryMap[key] || 'all'
        const active = activeCategory === cat && ['geopolitics','iraq','forex','calendar','crypto','oil','stocks','banks'].includes(key)
