...
export default function AdvancedSearch({ t, filters, setFilters, options }){
  ...
  return <section className="advanced-search-panel">
    ...
    <div className="advanced-grid">
      ...
      <label><span>{t.country || 'Country'}</span><select value={filters.country} onChange={e=>update('country', e.target.value)}><option value="all">{t.all}</option><option value="iraq">{t.iraq}</option><option value="iran">{t.iran}</option><option value="global">{t.global}</option><option value="usa">{t.usa}</option><option value="europe">{t.europe}</option><option value="china">{t.china}</option></select></label>
